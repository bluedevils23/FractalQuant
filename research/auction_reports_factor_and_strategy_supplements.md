...
生成器现已提供对应的 `*_mean_20d` 研究列，采用 `inclusive20` 口径：含当日竞价值和此前 19 个有效交易日。该值在完整竞价撮合结果到达后（通常 09:25）才可用；报告未明确是否含当日，因此不能把它解释为 09:25 之前的预测变量。仍建议在策略实验中与严格的 `prior20` 变换做敏感性对照：

```text
inclusive20 = rolling(20).mean()              # 生成器已实现，含当日
prior20     = shift(1).rolling(20).mean()     # 策略研究时另行派生
```
...
1. 基线：当前 76 auction 因子；
2. 实验 A：基线 + 13 个公开原子字段的 `inclusive20` 均值（策略端传 `--include-report-smoothed-features`）；
3. 实验 B：实验 A + 同字段的 `prior20` 变换；
4. 实验 C：对比只使用原子字段、只使用平滑字段和两者同时使用；
...
最有把握的新增价值来自长江 2024 的公开人工特征和广发 2014/海通 2019 的阶段一试盘极值。本次已将其中 25 个口径明确、数据源具备的字段加入生成器，并新增 13 个 `inclusive20` 平滑列；基础 auction 因子数保持 76，启用平滑列后策略输入为 89 个，且没有改变 opening-auction 时间窗口。
